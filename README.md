...
- `Account:Read` – 读取账号信息
- `Zone:Read` – 读取域名列表（自定义域名需要）
- `DNS:Read` / `DNS:Edit` – DNS 操作（自定义域名需要）
- `R2 Bucket:Read` – 读取存储桶列表
- `R2 Bucket:Edit` – 创建/删除存储桶（如需变更）
- `R2 Object:Read` – 读取对象列表
- `R2 Object:Edit` – 上传/删除对象（如需变更）

**注意**：现有的 DNS-only token 缺少 R2 权限，调用 R2 API 会返回 `Authentication error`。需要前往 Cloudflare Dashboard 生成包含 R2 权限的新 token。
...
> "查看 pest-gg-app-prod 绑定了哪些自定义域名"
...
**上传文件**
> "上传 /Users/dede/Desktop/logo.png 到 pest-gg-app-prod 的 images/ 目录"
> "上传 report.pdf 到 my-bucket 的根目录"

**下载文件**
> "下载 pest-gg-app-prod 里的 images/logo.png 到 /Users/dede/Downloads/"
> "获取 my-bucket 里 file.txt 的公开下载链接"
...
> "给 pest-gg-app-prod 添加自定义域名 files.pest.gg"
> "删除 pest-gg-app-prod 的 files.pest.gg 域名绑定"

### 4.6 公开访问配置

> "启用 my-bucket 的公开访问"
> "关闭 my-bucket 的公开访问"
> "my-bucket 的公开访问地址是什么"

### 4.7 CORS 配置

> "查看 pest-gg-app-prod 的 CORS 配置"
> "给 my-bucket 添加 CORS 规则，允许 https://app.pest.gg 访问"
...
2. **强制确认**：任何创建/删除存储桶、上传/删除对象、添加/删除自定义域名、修改公开访问或 CORS 配置的操作，必须先展示变更摘要，获得显式 `yes` 确认后才执行。
